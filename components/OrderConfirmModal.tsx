  ...
  const confirm = () => {
    ...
    try {
      ...
      if (editSig) {
        ...
        // Adding is meant to be frictionless: confirm with a quick toast and close
        // immediately, so the guest can keep tapping dishes without dismissing a
        // "View bill / Keep browsing" step every single time (owner, 2026-06-14).
        window.dispatchEvent(new CustomEvent("lfh:toast", { detail: { message: `${qty} × ${item.title} added`, subtitle: "tap the cart when you're ready", kicker: "your order", variant: "success", icon: "🛒" } }));
        setOpen(false);
      }
    } catch (e) {
