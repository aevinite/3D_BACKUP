...
// ── HourStrip — the traffic grid for ONE day (owner 2026-08-01) ──────────────
// The owner asked for "the traffic graph that was looking pretty" on the daily report
// too. The real Heatmap is 7 days × 24 hours; on a single day six of those seven rows
// are empty, which is exactly the lonely-one-bar failure the dynamic-chart rule exists
// to prevent. So one day gets ONE row in the same visual language — same single-hue
// ramp, same tap-for-detail readout, same Orders⇄Revenue toggle (both numbers already
// travel in the payload, so flipping costs no refetch). The full grid is used as soon
// as the report covers more than a day.
export function HourStrip({ data, accent }: { data: { hour: number; orders: number; revenue: number }[]; accent: string }) {
  ...
  const ords = Array(24).fill(0) as number[];
  const revs = Array(24).fill(0) as number[];
  for (const d of data) if (d.hour >= 0 && d.hour < 24) { ords[d.hour] = d.orders || 0; revs[d.hour] = d.revenue || 0; }
  const vals = m === "revenue" ? revs : ords;
  const max = Math.max(1, ...vals);
  const peak = vals.indexOf(max);
  const shown = sel != null ? sel : peak;
    ...
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, marginBottom: 10, flexWrap: "wrap" }}>
        ...
        <span style={{ fontSize: 11.5, fontWeight: 700, color: "var(--text)", marginLeft: "auto" }}>
          {sel == null ? "Busiest · " : ""}{hrRange(shown)} · {ords[shown].toLocaleString("en-IN")} order{ords[shown] === 1 ? "" : "s"}{hasRevenue ? ` · ${inr(revs[shown])}` : ""}
        </span>
      </div>
      {/* height = 44 cell + 4 gap + 12 label row. ScrollX puts this on the inner box, so a
          0 here collapses the whole strip to nothing (caught in the browser 2026-08-02 —
          the panel rendered its toggle and readout above an empty space). */}
      <ScrollX count={24} per={26} height={64}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(24, minmax(0,1fr))", gap: 3 }}>
          {vals.map((v, h) => (
            <button key={h} type="button" onClick={() => setSel((s) => (s === h ? null : h))}
              title={`${hrRange(h)} · ${ords[h].toLocaleString("en-IN")} order${ords[h] === 1 ? "" : "s"}${hasRevenue ? ` · ${inr(revs[h])}` : ""}`}
              aria-label={`${hrRange(h)}, ${ords[h]} orders`}
              style={{
                height: 44, borderRadius: 6, cursor: "pointer", padding: 0,
                background: v > 0 ? `color-mix(in srgb, ${accent} ${Math.round(12 + (v / max) * 88)}%, transparent)` : "var(--bg)",
                border: sel === h ? `2px solid ${accent}` : "var(--border)",
              }} />
          ))}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(24, minmax(0,1fr))", gap: 3, marginTop: 4 }}>
          {/* Every third hour is labelled — 24 labels in a phone-width row is unreadable. */}
          {vals.map((_, h) => (
            <span key={h} style={{ fontSize: 9, color: "var(--muted)", textAlign: "center", fontWeight: 600 }}>
              {h % 3 === 0 ? hr12(h) : ""}
            </span>
          ))}
        </div>
