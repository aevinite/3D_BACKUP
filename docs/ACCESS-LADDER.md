> # ⚠️ HISTORY ONLY — THIS LADDER WAS RETIRED ON 2026-07-31
>
> The model described below is **no longer how the app works**. It is kept in full, exactly as
> it was written, because it records why things were built this way — do not delete it.
>
> **Do not follow it for new work.** The live model is **`docs/ACCESS-MODEL.md`**: a toggle
> exists only where the owner listed one, only the admin holds permissions, and
> `lib/accessTree.ts` is the single source of truth.
