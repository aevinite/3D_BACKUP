...
There are **182** `verify:*` / `test:*` commands in `package.json`. Each one exists because a specific
bug reached somebody's screen once. That is a real asset and a real problem at the same time: nobody
can hold 182 names in their head, so in practice a person runs none of them, or reaches for
...
| **a part of a split put on a TAB, the offline-layer beat, and the money identities** — `lib/paySplit.ts` (pay-later parts), `lib/userAuth.ts` (`requireRole`'s presence beat), `lib/tax.ts` / `lib/taxFiling.ts` arithmetic, `lib/clashCompare.ts` | `verify:t24b` ← sweep #7 / T24's own 432 checks (ids P26601–P27032), the companion to `verify:t24-money-rules`. It covers the ground that one does not: the pay-later part of a split (migration 364 — a migration file), the device's offline-layer report (migration 366), and the money identities run over **thirty thousand randomly generated bills** rather than worked examples — subtotal = taxed + untaxed, due = subtotal − discount + tax, a discount never above its own base, never a NaN. Deterministic (fixed seed), so a failure reproduces. | nothing | no |
| **the split refusals and the Tax/GST sheet, as a person actually sees them** — `lib/paySplit.ts`'s refusal sentences, `lib/taxFiling.ts` on the owner's Tax/GST screen | `verify:t24b-live -- --base <url>` ← the LIVE half of T24's sweep-#7 checks (ids P27033–P27098). READ-ONLY by construction: every write it attempts is one the server refuses, so it writes no row and cleans nothing up. It reads the Tax/GST sheet off the real screen and reconciles it BOTH ways — the tax lines back to the total, the total back to the tile, the effective rate back to collected ÷ taxable — which is the fault that screen was built to end. | a running app + one cached staff login | no |
...
| a script **two panels both load** (the "Who is this bill for?" box) | `verify:shared-panel-styles` ← its look has to live somewhere BOTH panels load, or the box renders unstyled on one of them. Narrow on purpose. Added by sweep #8 T11 without a row here. | nothing | no |
| **one answer to "which guest is this number?"** | `verify:one-phone-number` ← three files answered it and none agreed, so the same phone found a different customer depending on which screen asked. Added by sweep #8 T11 without a row here. | nothing | no |
| the printed DOCUMENTS, and every sentence that describes them | `verify:print-documents` ← 161 phases written after twenty items were found on this feature in one pass, none of which was "does printing work": a figure right in total and wrong in its parts (a ₹0 tax component), two date and two rate formats on one sheet, prose naming controls that were deleted (in the app, in the engineering record, and in the page a client opens at a printer), a generated .bat whose check could never be true, and a write nobody looked at. Proves properties of the GENERATED TEXT and the RENDERED document — and SKIPS out loud where the honest answer needs a Windows machine. | nothing | no |
| printing, end to end, on paper | `verify:printing-sweep` ← 501 phases: the helper's five verbs and every refusal, the whole who-prints matrix asked of the real server as the real people, the address book's refusals, REAL prints through three virtual thermal printers with the paper measured (length, ink edges, cuts, nothing chopped), big paper measured on the document, the switches on and off, and a complaint closing only its own printer | `.env.local` · app running · the three ZZ-Virt-* queues + `node scripts/sweep/virtual-printers.mjs` | **YES** (its own rows, deleted by id) |
...
| the LOOK of any panel — a colour, a size, a contrast, or anything that has to stay readable in BOTH skins | `verify:look-ink` ← catches the traps that make a colour silently wrong: a `var(--x, #hex)` fallback where `--x` is declared nowhere that document can see, so the hex wins in both skins and one of them is stuck with a value tuned for the other. Static half needs nothing running | nothing (static half) | no |
...
| the **Kitchen printing card** on the owner's Settings page, or anything else added to these three screens after sweep #6 | `verify:owner-s7` ← the static half of sweep #7's block (P21101–P21400, 300 checks). Most of it is the printing card, which migrations 336/338/341 added to `/owner/settings` **after** the first 500 phases were written and which therefore had never been checked once: that it renders only when printing is genuinely on (R36), translates every stored value into English, describes a computer as a computer and a screen as a screen, offers no control at all, and leaks no code text. Also the Menu page after its `entitledSubset` rewrite, the roster's search + disabled grouping, and a token audit that checks EVERY `var(--…)` these four files read rather than a remembered list. Prose is matched whitespace-flattened, because JSX wraps a sentence wherever the line ran out and four checks first failed on correct source. | nothing | no |
| the same new ground, DRIVEN — the printing card in every state it can be in | `verify:owner-s7-live -- --base <url>` ← 200 assertions (P21401–P21600). Both skins at 1280 and 360 dpr3, contrast **computed** and the icon-to-label gap **measured**; eight forced states read off the screen (no computer · one awake · one asleep · one that has never checked in · one waiting · seven waiting · automatic printing off · the live read refused). Also counts what each screen costs on open, idle, **hidden**, on return and after navigating away. Contexts **block the service worker** — `public/sw.js` caches `/api/owner/*`, so the second page in a context is served by the worker and a page-level route handler never fires; eleven checks first reported a fault that was only an answer never delivered. **Writes nothing**: every state is this browser's own request answered differently. | app running | no |
| the owner's Menu / Team / Settings **on the deployed site**, rather than on a branch | `verify:owner-live500` ← T13's second 500 (P52701–P53200), planned from scratch and driven against **https://3-d-backup.vercel.app** on the owner's instruction: *"after making it live, you have to do the test on the live site"*. A branch that passes is not a site that works — the deployed bundle is minified, served off a CDN, hydrated over a real network, and sharing one database with nine other terminals. Covers first load and the states an owner meets on a bad day (slow, failed, empty, retry), the printing card in every state it can take, the roster at both widths AND in both skins with contrast computed, what each screen costs open / idle / hidden / on leaving, and whether the three screens agree with each other. **Writes NOTHING** — no row, no entitlement flip; every forced state is this browser's own request answered differently, because other terminals verify on the same deployment. Enforces its own numbering: an id issued twice, or one outside the block, stops the run. | the live site (or `--base`) | no |
...
| the admin's **Recycle bin, Billing & plans and Usage & cost** screens | `verify:bin-billing-usage` ← sweep #8 T20's 580 phases over the three screens together, because they share one shape: a list that can be empty, a figure that must agree with the ledger under it, and an action that must say what it did. Added 2026-09-04 without a row here, which is what `verify:pointers` caught — the guard was real and findable only by grepping package.json. | nothing | no |
...
| the **web address a guest reaches a restaurant on** — `/r/<slug>/menu`, `/q/<code>`, the menu-data API, or anything else that resolves a slug | `verify:guest-address` ← a removed restaurant KEEPS its row and its slug (mig 309, so the retained bills have something to hang off) and a new restaurant may take the freed name (mig 319), so two rows can hold one address. The resolver's `LIMIT 1` had no order, so it could answer with the dead one and the new restaurant's own menu 404'd — intermittently, which is why reading the answer once proves nothing. Mig 370 puts the live row first; this checks the ORDER BY is still there, that the partial unique index the reasoning stands on is still partial, and that `lib/tenant.ts` still nulls a closed restaurant itself. `-- --base <url>` also drives the whole loop, making and removing its own `zzguestaddr` rows by id. | nothing (`--base` for the live half) | no |
...
| ↳ the same two screens, but the WRITES and the FAILURES | `verify:repair-r2 -- --base <url>` ← round 2 of the above, and deliberately the ground round 1 could not reach: every button that changes something PRESSED for real and the database read back, each of the nine feeds MADE to fail / answer 401 / time out and the screen read, the shapes this stack is never in (empty, capped at 50, 200 broken 3D dishes, Arabic and emoji names, a gateway page as an error), keyboard reachability, a busy server, two tabs, and the fixes re-read on the DEPLOYED site. **It WRITES** — `scripts/sweep/t18r2/fixture.mjs` tags every row it makes, deletes it by id in a `finally` and on SIGINT/SIGTERM, and reads it back to prove it is gone. Bulk actions run only against a restaurant with no reports of its own (`findQuietRestaurant`), because a bulk action's scope is the SCREEN's, not the fixture's. | app running + `.env.local` | **YES** |
| the admin's Recycle bin, Billing & plans or Usage & cost — round 2 | `verify:bin-billing-usage-r2 -- --base <url>` ← T20's second round over those three screens. Registered here by T18 on 2026-09-05: it reached `main` without a row, which makes `verify:pointers` red, and that guard runs in the PostToolUse hook — a red one refuses Write/Edit for every session in this repo, not just its author's. | app running + `.env.local` | see its own header |
...
| the **Bills record screen** — the list, the bill beside it, the search box, or how a bill's money is shown | `verify:bills-screen` ← the owner picked this layout from three mock-ups (2026-08-22). It keeps: the list-beside-the-bill shape, ONE search box (no type dropdown) that matches every field including the last digits of an invoice, a SPLIT bill showing its parts (`session_payments` → `o.pay_parts`) with a pay-later leg told apart from money taken, BOTH the bill no. and the invoice no. each able to say it has none, no dead buttons, and the totals coming from the shared stack rather than arithmetic of their own | nothing | no |
| a GUEST or STAFF-PANEL api route (`app/api/guest`, `app/api/editor`, `app/api/kitchen`, `app/api/tablet`) | `verify:panel-api` ← the scoping, gating and shape rules the T10 sweep put back, so they cannot quietly come back out | nothing | no |
...
| a call to a DATABASE function (`.rpc(…)`, or a local `rpc()` wrapper like lib/session.ts's) | `verify:rpc-contract` ← get an argument name wrong and PostgREST does not say "wrong argument": it answers a 404 reading *"Could not find the function public.lfh_join_session(p_tabel, …) in the schema cache"*, at runtime, on a guest's phone. A rename in a migration and a rename in TypeScript are two separate edits. 188 call sites across 112 functions; every name must exist in a migration and every `p_*` argument must be declared | nothing | no |
| a read or write inside `lib/` that touches ONE restaurant's rows | `verify:scoped-reads` ← `lib/supabaseAdmin.ts` is the service-role client, so the database's row-level rules do not apply to it and the WHERE clause is the only scope there is. Every `.from("<tenant table>")` chain in `lib/` must name `restaurant_id`; the 21 platform-wide lookups (a login by username, a globally unique helper token) are exempt BY NAME with the reason, and a stale exemption is itself a failure | nothing | no |
| a month of stock (owner → Inventory, and the Inventory report) | `verify:inventory-window` ← `^\d{4}-\d{2}$` is the SHAPE of a month, not a real one: `?month=2026-13` used to answer with January 2027's purchases under that heading. Asserts the properties — every label is a real month, an impossible one falls back to the CURRENT month, every window is whole days from 05:00 IST, consecutive months touch exactly, and no caller carries a looser pattern | nothing | no |
...
| a route deciding what an OWNER may SEE in the activity log | `verify:log-visibility` ← finding F23: the switches were read by hand and the read's error was never checked, so a blip left the map empty and EVERY row passed, including the kinds Aevidine had switched off. Come through `lib/logVisibility.ts` — `loadLogVisibility()` answers `{ ok:false }` rather than an empty map, and `canSee()` hides what it could not check | nothing | no |
| a shared `lib/` helper building a person's message out of a database error | `verify:plain-refusals` ← the panel prints it as-is, so a PostgREST sentence lands in front of a manager mid-service. /api/maintenance was fixed for exactly this on 2026-08-05 and `lib/ownerScope.ts` → `dbFail()` is the worked example: log the detail, answer a sentence they can act on | nothing | no |
...
| the manager panel's SHELL or its host — `public/panels/editor/index.html`, `app/manager/**`, `app/editor/page.tsx`, `components/PanelFrame.tsx`, `lib/safeAreaBridge.ts` | `verify:panel-shell` ← the shell is markup nothing type-checks and no test opens, and every fault found in it was the same shape: a line still describing a world the code moved on from. It shipped the highlight on the Editor tab while app.js opens the FLOOR; it shipped the rail button's word "Collapse" beside its own "Expand menu" label; it named `.bill-amt` in the auto-fit list four weeks after that class was renamed; and `/editor` dropped the admin's person pin that `act-as/go` appends for exactly that path. So this compares the shell against `editor/app.js`, `panelGate.ts`, `fitnums.js`, `act-as/go` and the panel's own stylesheet — never against itself | nothing | no |
...
| anything under `app/kitchen/**`, `app/api/kitchen/[...path]/route.ts` or `public/panels/kitchen/**` | `verify:kitchen` ← the kitchen screen's own guard: it replays the 1,660 permanent numbered checks that territory already has, so a change there is measured against every rule the product has ever written down for it rather than re-read by eye | nothing | no |
...
| …the same shared panel files, judged against the LEDGER — every code-read row T9 and T4 carry about them, re-runnable one at a time (`-- --only P04034`). Reads files only; signs in to nothing | `verify:t12-plumbing` | nothing | no |
| …and the same territory DRIVEN in the real manager, kitchen and waiter panels — the pill, the popover's on-screen clamp, both skins, the undo card's window, the back-button depth and the crash logger. Writes nothing: it drives the DOM and uses the queue's own test-only pause hook rather than putting an order on a shared floor | `verify:t12-plumbing-live -- --base <url>` | app running | no |
...
| **a COUNT written into a rulebook** — how many page routes / admin API routes / admin-console pages there are, which Next or React is installed, how many guards exist — or a NEW `app/api/<family>` route, a NEW document in `docs/`, or a code path NAMED in any live rulebook | `node .github/scripts/verify-doc-counts.mjs` ← 20 counts across `CLAUDE.md`, `README.md`, `docs/CLAUDE-DETAIL.md`, this map and `docs/SECURITY-CHECKLIST.md`, plus: every admin route really carries the gate, and every API family is named in the Security-gate section (three were not, and all three were fine — a list that says it is COMPLETE sends the next audit at a correct route when it is not). Plus, since the sweep-7 pass: every document in `docs/` has a row in `docs/README.md` (nine did not, including the security checklist and both printing documents), and every code path named in a live rulebook still exists or is a RECORDED deletion (two were written as though still there). Repo-only, no key, runs in CI on every push. | nothing | no |
| **a panel page that exists at BOTH addresses** — `app/manager/page.tsx` and `app/r/[restaurant]/manager/page.tsx`, and the kitchen and tablet pairs | `node .github/scripts/verify-twin-route-parity.mjs` ← each panel must name its browser tab, its twin must agree, none may fall back to the root layout's generic name, **and both addresses must build the iframe URL through `panelIframeSrc()`** rather than by hand (a hand-built URL carries `rid` and silently drops `view=real` and `as=<staff id>`, so the same tab behaves differently at the two addresses). The T15 sweep named the tab on one side only, so the three tabs a restaurant's own staff switch between during a rush were all identical. Repo-only, no key. Companion to `verify:twins`, which covers the panels' behaviour rather than their route files. | nothing | no |
| **the sweep's permanent test record** — anything under `.claude/sweep/LEDGER/`, including `INDEX.md` | `verify:ledger-index` ← the record is only worth something if ONE ID MEANS ONE CHECK, FOREVER. It fails when two phase rows share an id, when a ledger is missing from the index, when the index cites a ledger that is not there without saying so, when a recap table references an id nothing carries, or when the index stops telling the next sweep to RE-RUN before it re-invents. A phase row is identified by its SHAPE (five cells), never by its first column — a first-column id in a narrative recap table is a back-reference, and treating those as duplicates is how 18 faults that did not exist got filed once. Split on UNESCAPED pipes only. | nothing | no |
| **`next.config.ts`, `vercel.json`, `tsconfig*.json`, `.vercelignore`, or `package.json`'s own scripts** — the files that decide what the browser is told and where the app runs | `node .github/scripts/verify-root-config.mjs` ← 24 checks. The response headers every visitor gets; that the content policy is still REPORT-ONLY (he parked flipping it on 2026-08-16 — enforcing it blanks a screen mid-service); that HSTS is production-only and carries no `preload`; that the eight framed-refusing surfaces are real routes and the GUEST menu is still deliberately not one of them; that an outside image host is granted only if something loads from it; Mumbai; `must-revalidate` on the service worker; the anchored `/editor` line that once 404'd the manager panel; strict TypeScript. **Until the sweep-7 pass nothing in this repo read `next.config.ts` at all.** Repo-only, no key, runs in CI on every push. | nothing | no |
| **`app/pair/page.tsx` / `app/api/pair/route.ts`** (the Allow page a printer's computer opens for itself), **`app/error.tsx` / `app/global-error.tsx`** (the two crash pages), **`app/layout.tsx`**'s first paint, or the tenant menu / dish pages' own default light-dark | `node .github/scripts/verify-unowned-routes.mjs` ← the pages no sweep territory names, so nothing checked them on purpose. Only the admin or a manager with `print_setup` may allow a machine, and a manager only ever into their own restaurant; the Allow page never handles the pairing secret; **no rule on it sets a `line-height` that a later `font:` shorthand throws away** (that is a SHORTHAND — it resets line-height to `normal`, and every button label on the page sat at the top of a 52px box because of it); both crash pages keep a way out, style themselves, follow the device's own skin and name no restaurant; the light/dark choice is still applied before the first paint; **and the tenant DISH page still repeats the restaurant's own default light/dark**, which it renders outside `AppShell` and therefore has to do by hand (a cold load of a dish URL came up light on a dark-default restaurant for months, because tapping through from the menu hides it). Repo-only, no key, runs in CI on every push. | nothing | no |
