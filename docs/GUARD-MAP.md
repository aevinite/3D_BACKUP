...
| **`app/pair/page.tsx` / `app/api/pair/route.ts`** (the Allow page a printer's computer opens for itself), **`app/error.tsx` / `app/global-error.tsx`** (the two crash pages) or **`app/layout.tsx`**'s first paint | `node .github/scripts/verify-unowned-routes.mjs` ← the pages no sweep territory names, so nothing checked them on purpose. The console link on the Allow page is shown only to the ADMIN (the printing board is in two places — mig 367 — and `/aevinite` answers a manager with a password prompt); a manager is sent to their own Settings → Printing and told so in words; only the admin or a manager with `print_setup` may allow a machine, and a manager only ever into their own restaurant; both crash pages keep a way out, style themselves, follow the device's own skin and name no restaurant; the light/dark choice is still applied before the first paint. Repo-only, no key, runs in CI on every push. | nothing | no |
