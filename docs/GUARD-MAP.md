...
There are **151** `verify:*` / `test:*` commands in `package.json`. Each one exists because a specific
bug reached somebody's screen once. That is a real asset and a real problem at the same time: nobody
can hold 151 names in their head, so in practice a person runs none of them, or reaches for
...
| **dropped, renamed or hand-edited any database object** (a table, a column, a function, a view, a trigger, an index, a read policy) | `verify:migration-truth` ← every object a migration file creates must still be in the database, or be retired later in the sequence. Nothing else asked this: `verify:db-parity` compares the two databases to each other, so both can be wrong together, and `verify:grants` is keyed by function NAME so it cannot notice an absence at all. Migrations 296/297 are the scar — a function was reported missing, re-created, and had to be un-resurrected | `.env.local` | no |
