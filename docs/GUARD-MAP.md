...
There are **141** `verify:*` / `test:*` commands in `package.json`. Each one exists because a specific
bug reached somebody's screen once. That is a real asset and a real problem at the same time: nobody
can hold 141 names in their head, so in practice a person runs none of them, or reaches for
