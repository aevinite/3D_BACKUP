# CLAUDE-DETAIL — the full text of every project rule

> **What this is (2026-08-05):** `CLAUDE.md` was 990 lines (~18K tokens) and loaded into
> EVERY session before the first word — past ~200 lines instruction-following measurably
> degrades ("context rot"), which is how always-on rules were getting skimmed. The always-on
> file is now a lean index of triggers and one-line rules; **this file holds the complete,
> unabridged text it compressed, under the same headings.** When the lean file's one-liner
> matches your task, open the matching section here (or the canonical doc it names) BEFORE
> acting. Nothing was deleted; this is the same content, loaded on demand instead of always.
...
- Next 16.3.4, App Router, async `params`. React 19.2.8. TS strict.
...
- **`/api/admin/**`** — every one of the 51 route files checks `tokenIsValid` (usually via a local
