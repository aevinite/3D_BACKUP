...
--
-- WHY THIS FILE EXISTS — a drift the owner's "check everything again" pass uncovered, and the
-- reason it matters more than the one line it carries:
--
-- lfh_table_view_summary is TIER 1 of the manager's live Table view. Comparing the two live
-- databases function by function showed the DEV database running a version that guards the dish
-- QUANTITY against a non-numeric value, while **no migration file contains that guard** — it had
-- been applied straight to the dev database by hand. So:
--   • the migrations folder was NOT the single source of truth it is supposed to be;
--   • AV LIVE could never receive the guard, because there was nothing to run;
--   • rebuilding dev from migrations would have silently REMOVED it again.
-- Without the guard, one order row whose items JSON carries a qty like "" / "1.5" / "2 pcs" makes
-- Postgres throw inside the summary — and a throw there means the manager's Table view stops
-- refreshing for the WHOLE restaurant (it is one query for every tile). That is exactly the
-- "the manager can't see what's happening" failure, from a single odd row.
--
-- This file is the dev database's current, working definition captured VERBATIM (pg_get_functiondef)
-- so both databases and the source now agree. Nothing else about the function changes:
--   qty  : CASE WHEN el->>'qty' ~ '^-?[0-9]+$' THEN (el->>'qty')::int END  (else default 1)
--   items: jsonb_typeof(b.items) = 'array' before jsonb_array_elements   (from mig 229)
--
-- Guarded from now on by `npm run verify:db-parity`, which fails when the two databases disagree
-- about any function/index/trigger outside the modules AV live deliberately doesn't have.
