-- 105_tile_counts_by_qty.sql
-- BUG (owner 2026-06-29): a floor tile showed a DIFFERENT "N cooking" count than the table's
-- detail view. e.g. table 14 had 2 order_item LINES both preparing, with quantities summing to
-- 4 — the tile said "2 cooking" but opening the detail said "4 cooking". Root cause: this
-- summary RPC counted dish ROWS (count(*) per status), while the panels' detail path sums the
-- per-line QTY (tablet tableAgg: `ck += r.qty`). Two different units → tile ≠ detail.
--
-- FIX: count by QUANTITY (plates), not by row, so nw/ck/rd/sv mean "how many dishes" — the
-- intuitive floor number and exactly what the detail already shows. The companion client change
-- (editor/app.js tableTileStateFromBoard → sum qty) keeps the manager's selected tile in lockstep
-- so PARITY IS LAW still holds: every tile, every panel, selected or not, now agrees on qty.
-- Only the DISPLAYED counts (+ the "x/y served" meta, which is built from these) change; the
-- state precedence / pay ring / due all key off >0 or the belong rows, so they are unaffected.
...
BEGIN
  ...
  LOOP
    ...
    -- is the fix: a single "2× Cappuccino" line is 2 cooking, not 1.
    lines AS (
