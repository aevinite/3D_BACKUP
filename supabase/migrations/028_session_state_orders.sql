-- Add the session's ORDERS (id, status, total, items, time) to lfh_session_state,
-- so EVERY member — not just the device that placed it — can track each order's
-- live timeline. Fixes: a guest orders, the guest sees the "Preparing…" tracker,
-- but the HEAD doesn't, because the floating tracker was device-local. Now the
-- head's tracker can pick up the table's orders from here. Purely additive.
