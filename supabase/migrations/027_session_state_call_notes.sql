-- Add the call REASON (note) to lfh_session_state's `calls` array, so the guest's
-- status widget can show what they've asked for ("You called: Water, Napkins").
-- Purely additive — every existing field is returned exactly as before.
...
BEGIN
  ...
  RETURN json_build_object(
    ...
    -- calls now include the `note` (the reason: Water, Napkins, Clean table…).
