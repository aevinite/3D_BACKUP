-- 136_table_summary_qty_restore.sql
-- REGRESSION (owner report 2026-07-06, proven live): the manager/tablet floor tiles that
-- are NOT selected under-count any multi-quantity dish. A "2× Cappuccino, cooking" tile
-- shows "1 cooking / 0/1 served" at rest, then JUMPS to "2 cooking / 0/2 served" the moment
-- you tap it — because the clicked tile, the detail head and the tablet all count by
-- QUANTITY (plates), while lfh_table_view_summary counts dish ROWS.
--
-- WHY: migration 105 (105_tile_counts_by_qty.sql) fixed exactly this by counting SUM(qty).
-- But migration 122 (the H9 discount-before-tax fix) was branched off migration 101 — which
-- predates 105 — and its header even says "the rest of the function is byte-identical to
-- migration 101". So 122 silently REVERTED 105's per-quantity counting back to count(*) rows.
-- 122 is the last redefinition, so the row-count version is what ships on prod.
--
-- FIX: keep migration 122's H9 due math (discount BEFORE tax via lfh_effective_tax_rate) —
-- that part is correct — and restore migration 105's SUM(qty) counting for nw/ck/rd/sv. Now
-- every tile (selected or not), the detail head, and the tablet agree on the plate count
-- again. This is the ONLY change vs migration 122: the dish-count CTEs + their SUM.
-- (Money/due, state precedence, meta, tail payload are all identical to 122.)
