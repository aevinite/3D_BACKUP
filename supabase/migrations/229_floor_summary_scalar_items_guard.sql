-- 229_floor_summary_scalar_items_guard.sql — one malformed order can't take down the floor view.
--
-- THE BUG (error log: "cannot extract elements from a scalar", tablet route, 2026-07-26 ×6):
-- lfh_table_view_summary expands each order's stored `items` JSON with
--   jsonb_array_elements(COALESCE(b.items, '[]'::jsonb))
-- COALESCE only replaces an SQL NULL. If a single row's `items` holds a JSON SCALAR instead of
-- an array (a JSON `null`, a number, a string — from a legacy row, a restore, or a replayed
-- offline write), jsonb_array_elements raises 22023 "cannot extract elements from a scalar" and
-- the WHOLE call fails. That call is TIER 1 of the Table view, so one bad row anywhere in a
-- restaurant blanks the entire floor grid for every manager and waiter, with a 500 they can do
-- nothing about. Proven against the dev DB: `select jsonb_array_elements('0'::jsonb)` returns
-- exactly that message. (Today every one of the 398,363 rows is a proper array, so this is
-- hardening a fragile read, not chasing current data.)
--
-- THE FIX: expand only when the value really IS an array; anything else counts as no items,
-- exactly like the SQL-NULL case already did. The write path already validates
-- (lfh_staff_place_order rejects a non-array p_items, mig 029/203) — this makes the READ side
-- equally unwilling to fall over.
--
-- The body below is the LIVE definition read back with pg_get_functiondef and patched in that
-- one place, NOT a copy of an older migration file — copying a stale copy is how a previous
-- fix got silently reverted here before.
