...
// plain text when the 2026-08-04 sweep found them, and nothing objected.
{
  ...
  // `stripLiveChecks` lived here (sweep #8 T12, 2026-09-05): it neutralised a live identifier
  // used as `/pattern/.test(...)`. T13 fixed the same guard the same week with the `searchable`
  // stripper below, which is the SUPERSET — it also clears a regex literal that is NOT followed
  // by `.test(` (the `const NEVER = [/3D_Menu_Av/, ...]` array form, which T12's version missed)
  // and clears comments too. One way, not two: the helper is gone and its only caller now uses
  // `searchable`. Both lanes were chasing the same red PostToolUse hook.
  const bad = [];
  for (const f of files) {
