...
import { readFileSync, writeFileSync, existsSync, statSync, mkdirSync, chmodSync, rmSync } from "node:fs";
...
# The breadcrumb below is the FIRST thing this file does: it is the only proof the window that
# opened actually ran this script. watch.mjs waits for it — see "a window that opened is not a
# session that started" further down.
: > '${started}'
...
// ── A WINDOW THAT OPENED IS NOT A SESSION THAT STARTED (owner, 2026-09-12) ────────────────────
// macOS does not execute a .command file directly — it opens a brand-new interactive shell and
// TYPES the file's path into it. That line can be damaged on the way in while the shell is still
// starting up, and on 2026-09-12 it was: the owner's window read
//   zsh: no such file or directory: s/Users/.../jobs/fix-260768ee.command
// One stray character, so the job never ran. The damage was not the dead window — it was the
// silence AFTERWARDS: the lock had already been claimed and the id already written to seen.json,
// so for the next FOUR HOURS every "Fix now" he pressed filed a ticket, popped nothing, and said
// "Sent to Claude" on the board. A launch that fails has to be loud, and it has to let go.
//
// So: the job leaves a breadcrumb on its first line, and we wait for it. Two tries, 15s each —
// two damaged type-aheads in a row is vanishingly rare, and a Mac that is genuinely refusing to
// open Terminal must not be handed a third window. If neither starts we release the lock (so the
// NEXT request is not blocked by a session that never existed) and mark the run failed, which is
// what puts a red row on the Repair page instead of nothing at all.
async function launch() {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    rmSync(started, { force: true }); // never trust a breadcrumb from an earlier try
    ...
    for (let waited = 0; waited < 15000; waited += 500) {
      ...
      if (existsSync(started)) {
        console.log(`popped terminal for request ${reqRow.id.slice(0, 8)}${attempt > 1 ? ` (took ${attempt} tries)` : ""}`);
        return;
      }
    }
    console.error(`window ${attempt} for ${reqRow.id.slice(0, 8)} opened but never ran the job — macOS damaged the command line`);
  }
  rmSync(lock, { force: true }); // nothing is running, so nothing may be blocked
  if (runId) {
    await fetch(`${url}/rest/v1/agent_runs?id=eq.${runId}&status=eq.running`, {
      method: "PATCH",
      headers: { apikey: key, Authorization: `Bearer ${key}`, "Content-Type": "application/json" },
      body: JSON.stringify({
        status: "failed",
        ended_at: new Date().toISOString(),
        report: "The Terminal window opened on the Mac twice, but macOS damaged the line that starts Claude both times, so nothing ran. Nothing is stuck and nothing is blocked — press Fix now again, or leave it for the 2:30 AM robot.",
      }),
    }).catch(() => {});
  }
}
