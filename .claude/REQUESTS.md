...
- **2026-08-19 · "THE SPACING IS NOT LIKE THE VIEW" — the preview was laying out across the window, not on the paper (owner, with a screenshot of the preview and a photo of the printed bill side by side)** — 3 items, done, live on backup.
  - **HE WAS RIGHT, AND IT WAS WIDTH.** The bill had **no width on screen**, so the preview laid itself out across the whole window (~780px) while the roll gives it **60mm of ink — about 227px**. Same font, a third of the room: "AANGAN GARDEN RESTAURANT" fitted one line in the preview and wrapped to two on paper, and every wrapped line made the print look tighter than the preview had promised. Nothing was wrong with the printer or the spacing rules — the preview was simply not showing the paper.
  - **THE PREVIEW NOW *IS* THE PAPER:** the bill window renders in the printer's own column — `width:70mm` + the same `padding:2mm 5mm` the print rule uses + **`box-sizing:border-box`** (without border-box the padding is added and the preview is 10mm wider than the roll — the same lie in smaller print) — on a grey desk with a paper shadow, with the Print/Close bar fixed to the window so it stays full size. **Measured after: column 70mm on both, content 60mm on both, and every block the same height — title 11.3mm, address 13.1mm, REPRINT band 9.9mm, item table 20.6mm, whole bill 117mm on screen and 117mm on paper.**
  - **ONE REAL TYPOGRAPHY FIX while in there:** the restaurant NAME was inheriting the body's 1.44 leading, so a wrapped name sat in 14.5mm of loose air at the top of every bill; it is 1.12 now (11.3mm) and a two-line name reads as one deliberate block. **The size is deliberately unchanged** — the name is the biggest thing on a customer's bill, and shrinking it to dodge a wrap would cost every restaurant legibility to save one line.
  - **CSS only. The markup is untouched**, because the same document is read inside the Audit card's sandboxed iframe with no scripts — a preview that needed JavaScript to look right would make the record and the paper two different things. `verify:print-format` now fails if the preview stops carrying the paper's column.
  - **Backup only**, as you said. Reprint one bill and it should match the preview line for line.
