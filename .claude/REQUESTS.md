...
- **2026-08-27 (evening) · ONE HELPER FILE FOR EVERY RESTAURANT, ZERO TYPING, AND IT STARTS ITSELF (owner: *"there wouldn't be one key for all restaurant. There would be different key for different restaurant or maybe a pairing code or whatever… one software only will be running in their PC just for printing"* → *"zero typing one, yeah"* → *"go, make the package"*)** — live on backup. **Where it lives:** a NEW page **`/pair`** (the Allow screen the helper opens by itself) · admin console → **Printing**, which now OPENS on **every restaurant, one row, worst first** · admin → Printing → **The helper file** card · manager panel → **Settings → Printing** (same file, same words) · `/print-setup.html` → new **"It is installed — now what?"** section · backend `print_pairings` (mig 368).
  - **HIS FIRST IDEA WAS ONE KEY FOR EVERY RESTAURANT, and he withdrew it himself.** It would have been refused anyway: one plaintext key on every client's PC means one leak prints at — and reads the bills of — every restaurant on the platform, and no shop could ever be cut off on its own. What he actually wanted was **one generic FILE**, and that is what he got.
  - **THE FILE NOW HOLDS NOTHING SECRET, so there is one of it, for ever.** It can be emailed, kept on a USB stick, copied off the screen. On its first run it describes itself, **opens the browser on its own machine**, and a signed-in person presses **Allow**. That is the whole setup: no code, no name, no password. The pattern is the one a smart TV uses to pair with Netflix.
  - **THREE THINGS MAKE IT SAFE, each with its own sweep phase:** the browser opens **on the machine at the printer** (that is the proof of "this is that computer") · the code is **not a credential** — alone it can only be shown to a logged-in human · the token is collected with a **secret only the helper holds, exactly once**, so even the person who approved it cannot read it afterwards. And the **restaurant is chosen by the approver, never by the helper** — a manager can only adopt into their own restaurant, and the request's restaurant id is ignored for them.
  - **THE MACHINE NAMES ITSELF.** His words were *"what the fuck is a computer name"*, and he was right — I read `Rishi's MacBook Pro` off his Mac with one command. That box is gone.
  - **IT STARTS ITSELF AFTER A SHUTDOWN — his other question.** A macOS **LaunchAgent with KeepAlive** (so it also restarts if it dies mid-service), a Startup-folder shortcut on Windows, a `.desktop` entry on Linux — all written **by the helper**, on every run. It used to be an instruction a person had to follow, so it was skipped; a skipped step means the shop opens, nothing prints, and nobody knows why. And a **single-instance lock**, because auto-start plus a double-click would otherwise put two helpers on one token.
  - **THE WINDOWS HOLE I HAD NEVER FLAGGED, closed.** The old script said *"put SumatraPDF.exe next to this file"* — quietly making the client download a program by hand, so *"nothing is downloaded"* was only ever true on a Mac. The helper now fetches it itself: **pinned URL, verified SHA-256** (checked by downloading the file), and a mismatch deletes it and refuses to run. **Windows also reports its paper sizes now** — it never did, so he was typing them, and paper size is the setting that decides whether a slip prints sideways or at half size.
  - **EVERY RESTAURANT ON ONE PAGE, which he predicted would be needed:** *"it will be messy when there will be too much restaurants."* 53 rows on the dev stack, **four whole-table reads for the whole platform** (never N+1), sorted worst-first — a pile-up beats a sleeping computer, which beats "no computer", which beats "not routed", and a shop that is not entitled sits quietly at the bottom. Click a row to set that restaurant up.
  - **AND HE WAS RIGHT ABOUT THE UI CLASHING.** On **Access & permissions**, the search results panel is capped at 560px while the cards behind run to ~1015px — so it covered the left half of every card and left the right half fully lit, chopping sentences mid-word (*"…no greyed-out leftovers"*, *"…ult — switch one on"*) with the count pills still glowing beside them. Two layers of text in one place. Fixed with a scrim that **dims AND blurs**: a dim alone measured rgb(16,20,27) → rgb(10,15,23) on the dark skin, which is a real change and invisible to a human. The results panel also widened, because the path a result lives at ("Manager › Permission for manager") was being ellipsised away — the one thing the list is FOR.
  - **REMOVED, being dead:** *"New code / shown only once"* on both boards (it minted a token to be carried by hand, and once the file stopped carrying one there was nowhere to show it — a button that mints a credential no screen displays is worse than no button) · *"Add a computer"* with its name box on the admin board · the panel's `printMade` state and `newcode` endpoint. **Unlink** replaces them all: unlink here, run the file there, press Allow — the same path as a first-time setup, and it empties the routes that named the machine.
  - **`managerCan()` moved to `lib/managerCan.ts`**, unchanged. A second door asks it now (`/api/pair`), and a permission rule with two copies is the bug class the access rebuild exists to remove.
  - **TWO FAULTS FOUND BY DRIVING IT, both mine, both before shipping:** (1) a **spent pairing answered "waiting" for ever** — the token had been collected and blanked, so the check fell through, and a helper restarted a moment after collecting would sit being told to wait with a perfectly good token on its disk (caught by sweep phase 104). (2) The **overview endpoint answered 400 to every request** — it sat below the route's `if (!rid) return err("Which restaurant?")`, and it is the one read with no restaurant behind it; the page rendered nothing and said nothing.
  - **RAN IT, not read it.** The generated Mac helper was executed in a sandboxed HOME (browser-open and `launchctl` stubbed so nothing touched his machine): it printed its status window, found *Rishi's MacBook Pro* and both its printers with paper sizes, started a pairing, and — after I pressed **Allow** in a real browser as the admin — collected its token, wrote it, installed the LaunchAgent, and reported *"Linked to My Little French House"*. Zero console errors on `/pair` in both states. `verify:print-helper` **81 → 100 checks**; `verify:printing-sweep` **100 → 112 phases, 0 failed**; typecheck clean; lint 0 errors. Test rows deleted by id.
  - **Still not witnessed, and I will not claim it:** the **Windows** helper has never been run — I have no Windows machine. Its paper-size and self-fetch code is written and guarded but unproven. And ink on paper from his Mac still needs the printer plugged in.
- **2026-08-28 · THE CONFUSING "WITH NO ANSWER ON THE KITCHEN SLIPS LINE" SELECT IS GONE — its meaning moved into the route first (owner: *"right now I don't understand three options… what do you mean by this option?"* → *"do what's left"*)** — live on backup. **Where it lives:** admin console → **Printing** → card 3 (the select is removed; the **Kitchen slips** line now carries all three of its answers, and a new **More → a backup screen** picker) · backend `settings.kot_print_target` retired by mig 369, `backupPanel` added to the route model.
  - **HE WAS RIGHT THAT IT WAS CLUTTER.** `kot_print_target` (mig 336) asked the SAME question as the Kitchen slips line, in older and vaguer words — and the two could contradict each other. The printing sweep caught exactly that on 2026-08-26: a route naming the manager screen was refused because an admin had set "kitchen" months before, so the owner's own choice did nothing.
  - **IT WAS NOT A DELETED DROPDOWN, and that is the whole point.** 2 of the 17 dev restaurants — **French House and Aangan, the two he actually tests with** — were on **`both`** with the Kitchen slips line never answered. `both` means "the kitchen screen prints, and a counter screen picks up anything it has left sitting for 30 seconds". Deleting the setting would have removed that safety net without a word. So **migration 369 carried every restaurant's value into the route first**, and only then did the setting go.
  - **A SCREEN ROUTE CAN NOW NAME A BACKUP SCREEN**, which is where `both` went — the mirror of the backup printer a computer route already had. `screenMayPrint` answers "yes, but only for slips older than 30 seconds", so the first screen always gets first refusal, and a screen cannot be its own backup (an age window nothing can satisfy looks like a rule and is not one).
  - **SEVEN FILES STOPPED READING IT:** the kitchen route, the manager route, the admin printing API, the admin settings form and its write route, the owner settings route and the Printing board. The **column stays** — schema changes here are additive and one folder feeds two databases — with a comment on it saying it is retired, and `verify:print-queue` now **fails if any of those seven touches it again**.
  - **TWO FAULTS FOUND BEFORE SHIPPING.** (1) **Mine:** with no route at all my first version let the MANAGER screen auto-print — the retired column defaulted to `kitchen`, so a manager panel left open at a restaurant that had never touched the Printing board would have started pulling kitchen tickets it never used to (sweep phase 22 caught it). (2) **The sweep's own:** its "a DIFFERENT person" shape read an owner row with **no restaurant filter**, so it could pick another restaurant's owner; the server correctly refused the shape and three phases blamed the product. And my new self-backup phase wrote the jsonb straight to the database, which never runs the validator it was testing — it goes through the real API now.
  - **Checked:** typecheck clean · lint 0 errors · `verify:static` 33/33 · `verify:print-helper` · `verify:print-queue` · `verify:access` · `verify:taps` · `verify:settings-columns` · `verify-doc-counts` · **`verify:printing-sweep` 112 → 118 phases, 0 failed**. Driven on screen in both skins: the select is gone, the Kitchen slips line reads "A screen (a person)" (carried across by 369), the backup picker is under More, and the kitchen board still derives its plain-words `"both"` so that panel's wording never changed.
- **2026-08-28 (evening) · TWO WAYS TO PRINT, ONE TOGGLE, AND ONLY THE CHOSEN ONE ON SCREEN (owner: *"I want both A and B — I want the toggle AND the simplify UI… and do one thing: toggle is like you only see the option you have selected, only option which is selected setting for that option will be shown"*, plus *"if we run that .bat or .command file it will open that Chrome which runs minimised and doesn't auto-open when printing required, doesn't affect other tabs while print — test all that by yourself"*)** — live on backup. **Where it lives:** admin console → **Printing** → card **2 · How this restaurant prints** (the toggle) and card **3 · The three papers** · manager panel → **Settings → Printing** (the identical card) · the new **print-station file** on both · `/print-setup.html` → new **"Two ways to print — pick one"** section · backend `settings.modules.printing.mode`, `lib/printStationScript.ts`.
  - **HE WAS RIGHT THAT CARD 3 WAS TOO MUCH.** It was three papers × (two shape buttons + a computer + a printer + a paper size + a screen + a person + a device + two backup pickers) with **FIVE Save buttons** — about twenty controls, every one on screen whether it applied or not. Now: **one toggle**, then **only that mode's setup**, then the three papers where each is just **On / Nobody** (plus a printer in computer mode). Measured on the running screen: **22 controls in computer mode, 14 in screen mode** on the console, and **14** on the manager panel. The refinements (paper size, backup printer) sit behind **More**.
  - **MODE B IS NEW — the restaurant's own Chrome, packaged like the helper.** One file per system, double-clicked once. It was always possible, but only as a wall of Terminal commands in the setup guide; now it is the same shape as the helper file.
  - **AND I TESTED IT ON YOUR MAC RATHER THAN ASSUMING — three things came out of that.** (1) Running Chrome directly steals focus, **and so does `open -g -j -n` with a real page** — measured, frontmost went Finder → Google Chrome. My first test used `about:blank` and passed, which would have shipped a false promise. So the launcher **remembers who had the screen and hands it straight back**: measured Finder → Chrome → Finder. (2) Its **own Chrome profile**, so their real Chrome, tabs, history and logins are untouched. (3) **The one nobody would guess:** a hidden Chrome is throttled by Chrome itself, and a throttled panel stops polling — the tickets would queue while everything looked fine. The three anti-throttling flags are load-bearing: a hidden instance carrying them **beaconed 13 times in 14 seconds** against a counter I ran. And it is `--kiosk-printing`, deliberately **not** `--kiosk` — that is fullscreen, the opposite of what you asked for, and it is what the old guide told people to use.
...
- [x] **Loyalty points, admin-switchable, built 2026-09-19** (owner: *"build it put first in the
  access and permisssion make toggle to on and off if it's on then only everything will change
