...
- [x] **Finish actions** — green **Pay now & print** (paid=true) + gold **Pay on pickup** (unpaid, sits in the Platform board with a 💰 Collect button). NOTE: "print" currently piggybacks the kitchen's existing KOT auto-print; a dedicated customer-receipt print is a small follow-up.
...
- [ ] **LATER (owner) — Tablet (waiter) panel**: same Parcel button + flow (header insertion point tablet app.js ~:565-575).
- [ ] **LATER (owner) — dedicated Admin on/off entitlement** (per restaurant, default OFF) like the Access ladder. For now the button rides the existing `take_orders` x-ray (hidden for staff without it, tinted for admin, server-enforced) — swap to a `parcel` flag when built.
- [x] **Verified** on the real manager panel as diagm1 (desktop + phone): button visible, modal correct (name/phone, both pay buttons, 59 dishes), Platform pills render; type-check clean. (Granted french-house managers `take_orders` so the demo shows it.) Non-#1 + tablet verification travels with the LATER items.
