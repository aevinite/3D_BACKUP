# SWEEP #9 — TERMINAL 25 of 40

**Your territory: The panel login routes and the remaining panel APIs**

`app/api/editor/[...path]/route.ts` **lines ~3,000 → end** (shared with terminal 24 by LINE RANGE), plus `app/api/panel-login`, `panel-logout`, `panel-profile`, `staff-login`, `staff-logout`, `app/login/**`, `app/staff-login/**`, `lib/userAuth.ts` (469). Never POST JSON to `/api/staff-login` while testing.

**The exact files you own** (re-derive this, do not trust the list):

```
app/api/panel-login/route.ts
app/api/panel-logout/route.ts
app/api/panel-profile/route.ts
app/api/staff-login/route.ts
app/api/staff-logout/route.ts
app/login/LoginForm.tsx
app/login/page.tsx
app/staff-login/BlockedView.tsx
app/staff-login/LoginForm.tsx
app/staff-login/page.tsx
lib/userAuth.ts
```

**Your ID block: `P104001` – `P104100`** — 100 ids, pre-allocated, yours alone.
**Your port:** 4425 — never 4000, that is the owner's own window.
**Your branch:** `sweep9/t25-panel-apis-and-login`   ·   **Your worktree:** `../wt-s9-t25`
...
- ❌ `cd /Users/aevinite/Documents/Projects/wt-s9-t25 && wc -l app/foo.tsx`
- ✅ `wc -l /Users/aevinite/Documents/Projects/wt-s9-t25/app/foo.tsx`
- ✅ `cd /Users/aevinite/Documents/Projects/wt-s9-t25 && npm run typecheck` — running a command is fine.
...
**The ledger files most likely to hold your rows: `T10.md` · `T27.md` · `T17.md` · `T11.md` · `T12.md` · `T13.md`.**
...
Fifty, with ids from **`P104001`–`P104100`**. Do not write 500; do not write 20. Fifty, chosen —
...
2. your **50** new checks are planned, executed and recorded with ids from `P104001`–`P104100`;
