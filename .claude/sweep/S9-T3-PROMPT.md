# SWEEP #9 — TERMINAL 3 of 40

**Your territory: The basket and placing an order**

`components/CartPanel.tsx` (1,211), `components/OrderTracker.tsx`, `lib/guestOutbox.ts` (864), `lib/menu.ts` (840), `app/api/guest/**`. The only guest write that is queued offline is place-order — do not rebuild the staff outbox on a misreading.

**The exact files you own** (re-derive this, do not trust the list):

```
app/api/guest/call-waiter/route.ts
app/api/guest/leave/route.ts
app/api/guest/limit-hit/route.ts
...
**Your ID block: `P101801` – `P101900`** — 100 ids, pre-allocated, yours alone.
**Your port:** 4403 — never 4000, that is the owner's own window.
**Your branch:** `sweep9/t3-basket-and-placing-an-order`   ·   **Your worktree:** `../wt-s9-t3`
...
- ❌ `cd /Users/aevinite/Documents/Projects/wt-s9-t3 && wc -l app/foo.tsx`
- ✅ `wc -l /Users/aevinite/Documents/Projects/wt-s9-t3/app/foo.tsx`
- ✅ `cd /Users/aevinite/Documents/Projects/wt-s9-t3 && npm run typecheck` — running a command is fine.
...
**The ledger files most likely to hold your rows: `T27.md` · `T3.md` · `T30.md` · `T28.md` · `T10.md` · `T15.md`.**
...
Fifty, with ids from **`P101801`–`P101900`**. Do not write 500; do not write 20. Fifty, chosen —
...
2. your **50** new checks are planned, executed and recorded with ids from `P101801`–`P101900`;
