# SWEEP #9 — TERMINAL 28 of 40

**Your territory: The owner server routes, and every remaining route**

all 13 of `app/api/owner/**` (5,352 lines), plus `app/api/log`, `health`, `maintenance`, `blocked`, `aggregators`, `print-agent` if unowned, and anything else under `app/api/` no other terminal names. `ownerScope()` gates every owner route.
...
app/api/owner/customers/route.ts
app/api/owner/inventory/route.ts
app/api/owner/issues/route.ts
app/api/owner/khata/route.ts
app/api/owner/oplog/route.ts
app/api/owner/overview/route.ts
app/api/owner/printing/route.ts
app/api/owner/ratings/route.ts
...
**Your ID block: `P104301` – `P104400`** — 100 ids, pre-allocated, yours alone.
**Your port:** 4428 — never 4000, that is the owner's own window.
**Your branch:** `sweep9/t28-owner-api-and-the-rest`   ·   **Your worktree:** `../wt-s9-t28`
...
- ❌ `cd /Users/aevinite/Documents/Projects/wt-s9-t28 && wc -l app/foo.tsx`
- ✅ `wc -l /Users/aevinite/Documents/Projects/wt-s9-t28/app/foo.tsx`
- ✅ `cd /Users/aevinite/Documents/Projects/wt-s9-t28 && npm run typecheck` — running a command is fine.
...
**The ledger files most likely to hold your rows: `T25.md` · `T10.md` · `T12.md` · `T13.md` · `T20.md` · `T28.md`.**
...
Fifty, with ids from **`P104301`–`P104400`**. Do not write 500; do not write 20. Fifty, chosen —
...
2. your **50** new checks are planned, executed and recorded with ids from `P104301`–`P104400`;
