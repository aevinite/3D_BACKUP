# SWEEP #9 — TERMINAL 4 of 40

**Your territory: The table session — a table shows only its own party**

`components/SessionGate.tsx` (1,159), `components/SessionStatusWidget.tsx` (507), `lib/session.ts`, `app/pair/**`, `app/api/pair/**`, `app/api/r/**`, `app/api/rt-config/**`. Ownership is the SESSION, never the table number; an order can never outlive its session.
...
app/api/rt-config/route.ts
app/pair/page.tsx
components/SessionGate.tsx
components/SessionStatusWidget.tsx
lib/session.ts
```

**Your ID block: `P101901` – `P102000`** — 100 ids, pre-allocated, yours alone.
**Your port:** 4404 — never 4000, that is the owner's own window.
**Your branch:** `sweep9/t4-table-session-and-who-you-are`   ·   **Your worktree:** `../wt-s9-t4`
...
- ❌ `cd /Users/aevinite/Documents/Projects/wt-s9-t4 && wc -l app/foo.tsx`
- ✅ `wc -l /Users/aevinite/Documents/Projects/wt-s9-t4/app/foo.tsx`
- ✅ `cd /Users/aevinite/Documents/Projects/wt-s9-t4 && npm run typecheck` — running a command is fine.
...
**The ledger files most likely to hold your rows: `T27.md` · `T3.md` · `T30.md` · `T10.md` · `T11.md` · `T12.md`.**
...
Fifty, with ids from **`P101901`–`P102000`**. Do not write 500; do not write 20. Fifty, chosen —
...
2. your **50** new checks are planned, executed and recorded with ids from `P101901`–`P102000`;
