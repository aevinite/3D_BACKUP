# SWEEP #9 — TERMINAL 5 of 40

**Your territory: Guest chrome, offline, every language — AND every remaining shared component**

`components/AppShell.tsx`, `public/sw.js`, `public/offline.html`, `lib/i18n.ts` (604), `components/RealtimeProvider.tsx`, `components/OfflineNotice.tsx` and **every remaining top-level `components/*.tsx`** — including the eight that no terminal in seven sweeps was ever told to check: `AutoFitNumbers`, `BackQuitDialog`, `FitNumber`, `PointerCaptureGuard`, `ToastHost`, `VegIcon`, `PanelFrame`, `RealtimeProvider`. Assert the ONLINE path too — a fix that only works offline is a new fault.

**You own 40 files.** Re-derive the exact list yourself from the description above; the planning session proved every one of them is owned by you and by nobody else.

**Your ID block: `P102001` – `P102100`** — 100 ids, pre-allocated, yours alone.
**Your port:** 4405 — never 4000, that is the owner's own window.
**Your branch:** `sweep9/t5-guest-chrome-and-every-shared-component`   ·   **Your worktree:** `../wt-s9-t5`
...
- ❌ `cd /Users/aevinite/Documents/Projects/wt-s9-t5 && wc -l app/foo.tsx`
- ✅ `wc -l /Users/aevinite/Documents/Projects/wt-s9-t5/app/foo.tsx`
- ✅ `cd /Users/aevinite/Documents/Projects/wt-s9-t5 && npm run typecheck` — running a command is fine.
...
**The ledger files most likely to hold your rows: `T1.md` · `T4.md` · `T2.md` · `T29.md` · `T27.md` · `T3.md`.**
...
Fifty, with ids from **`P102001`–`P102100`**. Do not write 500; do not write 20. Fifty, chosen —
...
2. your **50** new checks are planned, executed and recorded with ids from `P102001`–`P102100`;
