# SWEEP #9 — TERMINAL 39 of 40

**Your territory: THE REPO'S OWN 167 GUARDS — alive, honest, cleaning up?**

`scripts/**` (245 files, 209 `verify-*.mjs` at the root alone), `tests/**`, `.github/scripts/**`, `.github/workflows/**`, and all 167 `verify:*` / `test:*` entries in `package.json`. Judge a guard by READING it, never by a grep. Watch for stale ALLOWANCES, not dead scripts — that is what every red one has turned out to be. Does each one delete the rows it wrote, by id, and restore on kill?

**You own 269 files.** Re-derive the exact list yourself from the description above; the planning session proved every one of them is owned by you and by nobody else.

**Your ID block: `P105401` – `P105500`** — 100 ids, pre-allocated, yours alone.
**Your port:** 4439 — never 4000, that is the owner's own window.
**Your branch:** `sweep9/t39-the-repos-own-guards`   ·   **Your worktree:** `../wt-s9-t39`
...
- ❌ `cd /Users/aevinite/Documents/Projects/wt-s9-t39 && wc -l app/foo.tsx`
- ✅ `wc -l /Users/aevinite/Documents/Projects/wt-s9-t39/app/foo.tsx`
- ✅ `cd /Users/aevinite/Documents/Projects/wt-s9-t39 && npm run typecheck` — running a command is fine.
...
**The ledger files most likely to hold your rows: `T28.md` · `T29.md` · `T30.md` · `T9.md` · `T12.md` · `T14.md`.**
...
- `every script added under scripts/ since 2026-09-01 — re-derive with: git log --diff-filter=A --since=2026-09-01 --name-only --pretty=format: -- scripts | sort -u`
...
Fifty, with ids from **`P105401`–`P105500`**. Do not write 500; do not write 20. Fifty, chosen —
...
2. your **50** new checks are planned, executed and recorded with ids from `P105401`–`P105500`;
