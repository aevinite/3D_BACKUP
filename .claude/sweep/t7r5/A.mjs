...
try {
  ...
  // ── items 22–25 · the four that shipped since the fourth pass ──────────────────────────
  {
    const liveCss = await (await fetch("https://3-d-backup.vercel.app/panels/tablet/style.css")).text();
    const acceptRule = (liveCss.match(/\.accept \{[^}]*\}/) || [""])[0];
    C("item 22 — the live stylesheet gives ✓ Accept a 44px minimum", /min-height:\s*44px/.test(acceptRule), (acceptRule.match(/min-height:[^;]*/) || ["(none)"])[0]);
    // The detail has to be OPEN to measure its controls — the item-16 probe above lands the tab
    // back on the floor, so this read an empty list and failed on its own precondition.
    await openTable(s, "18");
    const live22 = await s.fr.evaluate(() => [...document.querySelectorAll(".detail-pop button")].map((x) => ({ t: (x.innerText || x.id || "").replace(/\s+/g, " ").trim().slice(0, 18), h: Math.round(x.getBoundingClientRect().height) })).filter((x) => x.h > 0));
    // TWO FAMILIES, TWO SIZES, both deliberate. The detail's ACTION buttons are 44px (item 22).
    // The controls on a DISH ROW are a 40px set — − and + are 40px squares, 🗑 became one in item
    // 25 and ✓ Serve joined them in item 26 — because a row where one control towers over the
    // others is worse than a row of forty-somethings. Judging ✓ Serve by the action row's rule
    // asked it to be the tallest thing on its own line.
    const rowKids = await s.fr.evaluate(() => [...document.querySelectorAll(".iline button")].map((x) => (x.innerText || x.id || "").replace(/\s+/g, " ").trim().slice(0, 18)));
    const worded = live22.filter((x) => x.t.replace(/[^A-Za-z]/g, "").length >= 3 && !rowKids.includes(x.t));
    const rowBtns = live22.filter((x) => rowKids.includes(x.t));
    C("item 22 — every worded ACTION button on the detail renders at 44", worded.length > 0 && worded.every((x) => x.h >= 44), worded.filter((x) => x.h < 44).map((x) => `${x.t}:${x.h}`).join(", ") || `all ${worded.length} at 44+`);
    C("item 26 — and every control on a DISH ROW is the same 40px as its neighbours", rowBtns.length === 0 || rowBtns.every((x) => x.h >= 40), rowBtns.map((x) => `${x.t}:${x.h}`).join(", ") || "no dish-row controls on this table");
    C("item 26 — …with none of them left smaller than the rest of the row", rowBtns.length === 0 || (Math.max(...rowBtns.map((x) => x.h)) - Math.min(...rowBtns.map((x) => x.h))) <= 4, rowBtns.map((x) => x.h).join(","));

    const liveJs = await (await fetch("https://3-d-backup.vercel.app/panels/tablet/app.js")).text();
    const clashes = [...liveJs.matchAll(/clash\.plain/g)].map((m) => liveJs.slice(m.index, m.index + 160).split("\n").slice(0, 2).join(" "));
    C("item 23 — every place a clash is reported shows the TODO as well", clashes.length >= 4 && clashes.every((c) => /todo/.test(c)), `${clashes.filter((c) => !/todo/.test(c)).length} of ${clashes.length} without`);

    const tileBlock = liveCss.slice(liveCss.indexOf(".t-top {"), liveCss.indexOf("/* ── SHED DETAIL ON THE WAY DOWN"));
    const floors = [...tileBlock.matchAll(/font-size:\s*clamp\(\s*([\d.]+)px/g)].map((m) => Number(m[1]));
    C("item 24 — no text on a tile can fall below 9px", floors.length > 0 && floors.every((f) => f >= 9), `floors: ${floors.join(", ")}`);
    C("item 24 — …and the WORDS on it are at least 10px", /\.t-take \{[\s\S]{0,400}?font-size:\s*clamp\(\s*10px/.test(liveCss) && /\.t-line-plain \.t-linenum \{ font-size: clamp\(10px/.test(liveCss), "＋ Take order and the served line");
    const smallest = await s.fr.evaluate(() => {
      const xs = [...document.querySelectorAll(".tile, .tile *")].filter((e) => !e.children.length && e.innerText.trim()).map((e) => parseFloat(getComputedStyle(e).fontSize));
      return xs.length ? Math.min(...xs) : null;
    });
    C("item 24 — …and nothing on the rendered floor is under 9px", smallest === null || smallest >= 9, `smallest ${smallest}px`);
    const idel = (liveCss.match(/\.idel \{[^}]*\}/) || [""])[0];
    C("item 25 — 🗑 is a 40px square, like the − and + beside it", /width:\s*40px/.test(idel) && /height:\s*40px/.test(idel), (idel.match(/width:[^;]*;\s*height:[^;]*/) || ["(none)"])[0]);
  }
