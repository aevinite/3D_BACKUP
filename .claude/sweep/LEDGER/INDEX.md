...
**Next free ID: `P46801`.** (T18 took `P46301`–`P46800` for its second, freshly planned 500 on 2026-08-31; `P46735`–`P46800` inside that block are UNUSED and free — 434 of the 500 were written and executed, and block I's 73 were written but its harness would not complete.)
...
> | T18 · the admin's money view — a SECOND, freshly planned 500 | `P46301`–`P46800` | ✅ **434 of 500 written and executed (416 ✅ · 18 ❌)**; block I's 73 were written but its harness stalled three times and is recorded as NOT RUN; `P46735`–`P46800` unused and returned. NEW ground in the same six files: what each screen does when the SERVER answers badly, what happens when a person is fast, the figures reconciled against the database, keyboard-only use, the paging edges, every user-visible word, and what the screens ask the network for. **4 real faults found and fixed, one commit each:** two screens lost their whole page when one field went missing; one bill in an unknown state took the ledger down; a Customers page past the end came back as a red error; and item 12's own fix turned out to cover one field of four. **13 of the 18 ❌ were the CHECK being wrong** — most usefully, the service worker caches /api/admin/* so fault injection cannot reach a cached admin endpoint at all. |
>
