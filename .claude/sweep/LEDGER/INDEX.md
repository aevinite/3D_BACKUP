...
> | T18 · the admin's money view — a SECOND, freshly planned 500 | `P46301`–`P46800` | 🔄 CLAIMED 2026-08-31, in progress. Planned from scratch on his instruction ("plan the whole 500 phases test and execute it, make sure it should be in the boundary that you have been given"), so it is NEW ground inside the same six files rather than a re-run: what each screen does when the SERVER answers badly, every empty/loading/error state seen in both skins, interaction sequences, the four money screens reconciled against the database itself, keyboard-only traversal, the paging edges, every user-visible word, and what the screens ask the network for while being used. |
>
