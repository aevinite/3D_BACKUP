...
| 33 | P104801–P104900 (round 1) · P152001–P153000 (round 2+; **P152001–P152456 spent by round 2 on 2026-09-17** — 506 phases. Blocks B and C are GENERATED one row per function and one per file, so the total moves when the territory does: 396 added a file (+1), 397 added a file and brought a function into range (+4), and one realtime probe split in two when 397 changed what the right answer was (+1). A real check is not trimmed to hit a round number. Ids come from one allocator, `scripts/sweep/t33/ids.mjs`, rather than a hard-coded start per block — hand-nudging four starts is how an id collision happens and it nearly did twice in one afternoon. Nothing claimed from the *Next free ID* line, which was neither read nor edited) | **The database, migrations at POSITIONS 321 → THE END** (`ls supabase/migrations/*.sql \| sort \| sed -n '321,$p'` — 81 files, 12,898 lines, `314_a_settled_bill_…` → `393_the_last_fifteen_guest_tables_…`; a positional range is NOT a numeric one, so position 321 is the file named `314_…`). **60 of its 81 files had ZERO ledger rows by subject before this file existed** — every migration from `333_…` to `393_…` except two. `T33.md` (round 1, 50 rows) and `T33-S9-R2.md` (round 2, 506 rows — 59 of them DRIVEN: a real write inside a rolled-back transaction, which round 1 had none of), both filed 2026-09-17 (sweep #9) | ✅ | 556 | 555 | 1 | 0 |
