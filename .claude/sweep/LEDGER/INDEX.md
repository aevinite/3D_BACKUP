...
**Next free ID: `P53545`.** *(T26 took `P52545`–`P53044` (round 3) and `P53045`–`P53544` (round 2, renumbered the same day after T25 was found to own its original block on `main`) on 2026-09-01 — **renumbered from `P46801`–`P47300`, which T20 had claimed on 2026-08-31 and pushed to `main` first.** T20 keeps the block: its rows were already merged, and moving a merged id breaks every reference to it — the same call T11 made for T7. This is the sixth time this file has recorded that shape, and the rule it already states is the one that decides it: **a claim only counts once it is ON `main`.** My round-3 block was chosen from the TOP of free space and re-checked against every ledger as it exists on `origin/main` in the same breath as writing it.)*

**Previously (main's mark, kept):** **Next free ID: `P52545`.** *(T2 took `P52495`–`P52544` on 2026-09-01 — 50 rows, not 500: they
