...
| 33 | P104801–P104900 (round 1) · P152001–P153000 (round 2+; **P152001–P152451 spent by round 2 on 2026-09-17** — 501 phases, because block B+C grew by one row when migration 396 added a file to the territory mid-round, and a real check is not trimmed to hit a round number. Nothing claimed from the *Next free ID* line, which was neither read nor edited) | **The database, migrations at POSITIONS 321 → THE END** (`ls supabase/migrations/*.sql \| sort \| sed -n '321,$p'` — 81 files, 12,898 lines, `314_a_settled_bill_…` → `393_the_last_fifteen_guest_tables_…`; a positional range is NOT a numeric one, so position 321 is the file named `314_…`). **60 of its 81 files had ZERO ledger rows by subject before this file existed** — every migration from `333_…` to `393_…` except two. `T33.md` (round 1, 50 rows) and `T33-S9-R2.md` (round 2, 501 rows — 58 of them DRIVEN: a real write inside a rolled-back transaction, which round 1 had none of), both filed 2026-09-17 (sweep #9) | ✅ | 551 | 550 | 1 | 0 |
