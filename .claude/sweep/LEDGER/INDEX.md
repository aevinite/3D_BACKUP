...
> | T16 · the admin's restaurants, owners, settings, billing, bin & platform floor | `P22601`–`P23100` | ✅ 500 written, 500 executed (496 ✅ · 4 ❌, all four the same fault — see below) — plus all 500 of `P07501`–`P08000` re-run: **499 ✅ · 1 ❌**, one REGRESSION by drift (`P07771`, the delete cards still promised a 90-day protection the owner removed on 2026-08-20; fixed on the branch), 9 expectations moved with their reason written in the row, and two sweep-#6 rows found to have been **filed green on wording that was wrong or loose** (`P07950`, `P07924`). The 5 open ❌ are ONE database-layer fault outside this territory: `lfh_guest_restaurant` (mig 282) resolves a slug with `LIMIT 1` and no `deleted_at IS NULL`, so a restaurant created on a REUSED web address can serve a 404 on its own menu — `P07967`, `P23036`–`P23039`. |
