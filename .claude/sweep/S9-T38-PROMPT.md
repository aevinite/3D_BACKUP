# SWEEP #9 — TERMINAL 38 of 40

**Your territory: EVERY WORD ON EVERY SCREEN**

the `lib/i18n.ts` dictionary VALUES, and every user-visible string in `app/`, `components/`, `public/panels/`. Labels, buttons, errors, empty states, toasts. **You change only text strings — never layout or colour.** You share files with terminal 37 on purpose; the merge integrates both sides.

**Your ID block: `P105301` – `P105400`** — 100 ids, pre-allocated, yours alone.
**Your port:** 4438 — never 4000, that is the owner's own window.
**Your branch:** `sweep9/t38-every-word`   ·   **Your worktree:** `../wt-s9-t38`
...
- ❌ `cd /Users/aevinite/Documents/Projects/wt-s9-t38 && wc -l app/foo.tsx`
- ✅ `wc -l /Users/aevinite/Documents/Projects/wt-s9-t38/app/foo.tsx`
- ✅ `cd /Users/aevinite/Documents/Projects/wt-s9-t38 && npm run typecheck` — running a command is fine.
...
**Grep for your own file names across every `LEDGER/T*.md` — no one file is yours.**
...
Fifty, with ids from **`P105301`–`P105400`**. Do not write 500; do not write 20. Fifty, chosen —
...
2. your **50** new checks are planned, executed and recorded with ids from `P105301`–`P105400`;
