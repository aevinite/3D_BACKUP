...
//   (now − created_at, reported by the realtime hooks) and, on the poll-only owner
//   panel, how long its existing refresh already takes. Zero new requests.
...
// (now − created_at), and by the owner poll with how long its existing refresh took.
// No new request is ever made to obtain this. Garbage (negative / absurd) is dropped.
