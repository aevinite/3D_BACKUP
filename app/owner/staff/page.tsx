        ...
        return (
          <div key={r.id} className="adm-card ost-card" style={{ ["--rcol" as string]: r.accentColor }}>
                ...
                <div key={s.id} className={`ost-row ${s.active ? "" : "off"}`}>
                  ...
                  {editing?.id === s.id && (
                    <div className="ost-editrow">
                      <input className="ost-in" value={editing.name} autoFocus placeholder="Name (their login)" autoComplete="off" maxLength={80}
            ...
            <form className="ost-add" onSubmit={(e) => { e.preventDefault(); addStaff(r.id, e.currentTarget); }}>
              <input className="ost-in" name="name" placeholder="Name (their login)" autoComplete="off" maxLength={80} required />
