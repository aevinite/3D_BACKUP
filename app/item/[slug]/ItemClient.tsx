...
export default function ItemClient({ slug, fromCat, restaurantId, restaurantSlug }: { slug: string; fromCat?: string; restaurantId?: string; restaurantSlug?: string }) {
  ...
  // DOES THE VEG / NON-VEG MARK MEAN ANYTHING ON THIS MENU? (owner, 2026-08-12: *"there shouldn't be
  // a non-veg chip … because it's veg"* — the rule is that on a menu where every dish is on the same
  // side of the line, the chips AND the per-dish mark both go, because marking all 199 dishes green
  // says nothing.)
  //
  // components/MenuView.tsx has derived this since that day and passes it to each card as `showDiet`.
  // This page never got it, so on Aangan the grid showed ZERO marks and opening any dish showed one —
  // measured and screenshotted on the deployed site (guest sweep T1, 2026-08-16). Same derivation,
  // from the same list, so the two surfaces can only ever agree.
  //
  // Guarded on `allItems.length` exactly as MenuView guards on `menuData.length`: while the menu is
  // still loading we keep today's behaviour (the switch alone) rather than flickering the mark away.
    ...
    <div id="detail-page" className="page active item-detail-page">
      ...
      <div className="detail-visual" onClick={() => setImgZoom(true)} style={{ cursor: 'zoom-in' }}>
        ...
        {/* The veg / non-veg badge in the corner — same single switch as the menu cards
            (Access → Menu → Veg / non-veg) AND the same "does it mean anything here?" test they use,
            so a pure-veg restaurant genuinely shows no mark anywhere. See `showDiet` above. */}
        {showDiet && (
