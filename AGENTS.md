...
- **Keep the running request checklist at `.Codex/REQUESTS.md`** — add every owner
...
- **Settings / hooks / allowlist edits**: use the `update-config` skill —
  do not hand-edit `.Codex/settings.json` blindly.
- **Migrating Codex API or Anthropic SDK code**: use the `Codex-api` skill.
...
- **ABSOLUTE RULE — secrets never appear in chat, ever.** This includes the Supabase
  ...
  (1) ANY `Codex mcp ...` command echoes resolved secrets — ALWAYS redirect its
  output to null (`cmd /c "Codex mcp ... >nul 2>&1"`); (2) never `cat`/`Get-Content`/
...
- **MCP servers are NOT read from `.Codex/settings.json`.** Codex loads
  them from `~/.Codex.json` (via `Codex mcp add ... -s local`) or a root
  `.mcp.json`. The supabase MCP is registered in `~/.Codex.json` and uses
  ...
  needs a full Codex restart to take effect.
