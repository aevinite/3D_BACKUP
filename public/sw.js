...
const VERSION = "v14"; // v4: no false alarm. v5: a saved copy can't mask a change you just made. v6: the offline page names the real reason. v7: the last-resort page survives a sign-out. v8: the page you're ON is saved on the FIRST visit, and the offline page's re-checks are jittered. v9: a STAFF PANEL's first visit saves its reads too (it saved none), and the last-resort page no longer promises work it can't know was saved. v10: the offline page's way OUT suits who is looking — a diner is sent back to their restaurant's menu instead of the staff sign-in (the bump is required: /offline.html is precached, so devices keep the old copy until the cache names change). v11: v10 missed the 3D DISH VIEWER — /view/<folder> has no /r/<slug> in its path, so a diner who lost signal in the 3D view was still handed the staff sign-in. Same bump reason as v10: /offline.html changed. v12: the branded page names the restaurant when the device has it, and stops offering a second button that only repeats the first (/offline.html changed, so the bump is required). v13: the no-signal screen is the one he picked from the ten prototypes — a live signal meter that costs ZERO requests (navigator.connection plus the round trip of the probes this page already makes), and a small game to pass the time that stops itself the moment the connection is proven back. /offline.html changed substantially, so the bump is required. v14: that screen was then rebuilt around the signal itself — the pulsing dot and two of the three bordered boxes are gone, every verdict is one sentence, and the way out is readable (it was 1.00:1 against its own button on the staff 404). /offline.html changed substantially, and it is PRECACHED, so without this bump every device already holding v13 keeps the old wall-of-text copy for ever.
...
async function offlinePage() {
  ...
  return new Response(
    ...
    ' &nbsp;·&nbsp; <a id="home" href="/" style="color:#38bdf8">Go to the home screen</a></p></div>' +
    // AND THE WAY OUT SUITS WHO IS LOOKING, exactly as /offline.html now does it (guest sweep T1,
    // 2026-08-16). "/" is the STAFF sign-in (app/page.tsx redirects it to /login), which is the
