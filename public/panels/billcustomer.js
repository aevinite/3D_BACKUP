  ...
  function ask(opts) {
    ...
    return new Promise((resolve) => {
      ...
      function sync() {
        ...
        goBtn.disabled = !(required ? ok : (p.length === 0 || p.length === 10));
      }
      ...
      function showMatches(rows, p) {
        ...
        matchEl.innerHTML = list.slice(0, 4).map((r) =>
          `<button type="button" class="btn small bc-pick" data-p="${esc(norm(r.phone))}" data-n="${esc(r.name || "")}"
             style="display:block;width:100%;text-align:left;margin:3px 0">${esc(r.name || "No name")} <span class="muted">· ${esc(pretty(r.phone))}</span></button>`).join("");
      ...
      phoneEl.addEventListener("input", () => {
        ...
        phoneEl.value = d.length > 5 && d.length <= 10 ? d.slice(0, 5) + " " + d.slice(5) : d;
        sync();
        lookup(false);
      });
      phoneEl.addEventListener("keydown", (e) => { if (e.key === "Enter") { e.preventDefault(); nameEl.focus(); } });
      nameEl.addEventListener("keydown", (e) => { if (e.key === "Enter" && !goBtn.disabled) { e.preventDefault(); goBtn.click(); } });
