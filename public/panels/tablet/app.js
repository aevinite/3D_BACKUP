...
async function selectTable(t) {
  ...
  // Also drop any ADD-TO-ORDER / view-order / per-order EDIT state — it belongs to the table
  // we're leaving. Without this, "＋ Add dish" then tapping another tile left addToOrderId
  // pointing at the OLD table's order, so dishes meant for the new table were appended to the
  // old bill + kitchen ticket (audit 2026-07-09). editOrders/viewOrder are cleared for the same
  // reason so the new table never opens mid-edit on a stale order id.
...
function bindFloorDelegation() {
  ...
  if (tilesEl) tilesEl.addEventListener("click", async (e) => {
    ...
    if (tile) {
      // On a wide tablet the floor stays clickable beside the order panel, so an accidental
      // tile tap while building a NEW order would silently wipe the whole cart. Confirm first
      // (only when there are unsent items and it's a DIFFERENT table). (audit 2026-07-09)
...
function renderShiftPicker(t, s) {
  ...
  document.querySelectorAll("[data-shiftto]").forEach((b) => (b.onclick = () => {
    ...
    const from = state.table;   // where to snap back to if the move is refused
    dropLayer();
    runOptimistic(
      ...
      () => { state.table = from; },   // refused (target taken / closed) → stay on the ORIGINAL table
    );
  }));
}
...
async function runOptimistic(mutate, fn, rollback) {
  try {
  ...
  // On a REJECTED action (e.g. "Move table" refused because the target got taken), run the
  // optional rollback BEFORE reloading so the panel doesn't stay pointed at a table the party
  // never moved to — otherwise the waiter could mark-paid / close the WRONG party's bill
  // (audit 2026-07-09). load() then repaints from server truth.
  catch (e) { toast("Failed: " + e.message, false); if (rollback) { try { rollback(); } catch {} } }
